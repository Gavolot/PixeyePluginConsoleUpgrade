	...
	public class ComponentConsole : MonoBehaviour
	{
		...
		public void ActivateConsole(bool activating)
		{
			if (activating)
			{
				...
				//EventSystem.current.SetSelectedGameObject(fieldInput.gameObject);
				//fieldInput.OnPointerClick(null);
				if(!fieldInput.IsActive())
		...
		void Awake()
		{
			LayerApp.WaitFor(0.1f, () => EventSystem.current.SetSelectedGameObject(fieldInput.gameObject));
			//timerCheckEventSystem = new Timer(0.1f, () => EventSystem.current.SetSelectedGameObject(fieldInput.gameObject));
			processorConsole = LayerApp.Get<ProcessorConsole>();
		...
		void CheckEventSystem()
		{
			...
			if (evSystem == null)
			{
				...
				evSystem = LayerApp.Obj.Spawn("Event System").gameObject.GetComponent<EventSystem>();
			}
		...
		void RenderDebug(float fps)
		{
			...
			debugStr.Append($"FPS: {fps.ToString("F1")} Time: {LayerApp.Time.deltaTime}ms GPU memory: {SystemInfo.graphicsMemorySize}mb Sys Memory: {SystemInfo.systemMemorySize}mb")
				...
				.Append($"\nActors: {Every.Ecs.GetAliveAmount()} Ticks: {Kernel.GetTicksCount}");
		...
		IEnumerator _ColorLine()
		{
			...
			while (true)
			{
				...
				if (Kernel.IsQuittingOrChangingScene()) yield break;
